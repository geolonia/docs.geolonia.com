...
npm run lint:text # 手書きページの文章を検査する（後述の[文章の書き方](#文章の書き方)）
npm test          # textlint のカスタムルール自体の単体テスト
```

**手でデプロイするコマンドはありません。** 配信は GitHub 連携で自動的に行われます
（後述の[デプロイと配信ヘッダ](#デプロイと配信ヘッダ)）。

テストがあるのは `textlint-rules/` のカスタムルールだけです。**ページが正しく作られていることの
確認は `npm run build` が通ることで代用**します。後述のとおり、載せたコードに誤りがあると
ビルドが落ちるようになっています。

CI は `npm test` → `npm run lint:text` → `npm run build` → `npm run check:links` の順で
実行します。手元で同じ順に流せば、CI と同じ範囲を確認できます。
...
|---|---|---|---|
| `tutorials/` | 学ぶ | まだ何も分からない | 実際に動く一本道の手順。Step 0 から順に積む |
...
なお、**使い方の説明としてコードブロックの中に `<LiveCode>` を書いても問題ありません**。
...
import したりすると、その場でビルドが落ちます。読者に動かないコードを見せずに済む、
というのがこの仕組みの主目的です。
...
チュートリアルのように段階を追って足していくページでは、途中のコードは前の Step の続きです。
...
読み込みはスクロールして見えたときに始まるので、ページを開いた時点で全部の地図が
動き出すことはありません。

## 3つのライブラリを並べる：Lens

Geolonia Maps には embed / maps-suite / maps-react の3つのライブラリがあります。同じことを
...
選んだ実装方式は `localStorage` に保存され、**サイト全体で同期します**。embed を選んだ読者は
他のページに移動しても embed のままです。ページ内に複数のタブ群があるときも連動します。

その群に存在しない実装方式が選ばれている場合は、先頭のものが表示されます（保存された
...
`<Lens>` の中では使わないでください。どの実装方式を表示しているかはタブが示しているので、
ブロックの先頭に同じ名前のバッジを置くと語が重複します。

### QuadGrid

トップページのセクション案内カードです。中身は固定なので、増やすときは
...
ビルドとデプロイが自動で実行されます。手でデプロイするコマンドは置いていません。
...
submodule を clone できずに失敗していました（Pages は submodule に認証情報を渡しません）。
...
- `.md` の中の `<!-- コメント -->` はビルド時に取り除かれるので、書きかけのメモを
  残しておけます（`.mdx` では `{/* コメント */}`）。
- 画面上の位置で案内しないでください（「右上のタブ」など）。実装方式のタブは記事の中に
  出るので、「上のタブで切り替えられます」のように書きます。
- チュートリアルの進行は `Step 0` から `Step 5` と呼びます。ページの `title` は
  `'Step 3: 地図スタイルの適用'` のように、`Step 番号` + 半角コロン + 名詞句で書きます
  （コロンを含むので frontmatter では引用符が要ります）。サイドバーのラベルは
  `src/lib/doc-nav.ts` にあり、ページの `title` と揃える必要があります。

### 機械で検査している言い回し

`npm run lint:text` は3つの層で文章を検査します。エラーが出たら、メッセージが示す
書き方に直してください。

| 層 | 実体 | 見ているもの |
|---|---|---|
| 表記揺れ | `.textlint.prh.yml`（prh） | 製品名・技術用語の表記（`GeoJSON`、`MapLibre GL JS` など） |
| 言い回し | `textlint-rules/no-ai-tone.js`（自作） | 比喩・話し言葉・講義調・対話的な主語といった文体 |
| 語彙 | `preset-ai-words-ja` | AI が書いた文章に多い単語（形態素解析と辞書による判定） |

自作ルールの `no-ai-tone` が検出するのは次の4種類です。

<!-- 表の「例」列は検出対象そのものなので、この表だけ検査から外す。 -->
<!-- textlint-disable no-ai-tone, ai-words-ja/no-ai-words -->

| 種類 | 例 | 直し方 |
|---|---|---|
| 独自の比喩 | レンズ、段0、入口、梯子、横棒（─） | 実装方式、Step 0、はじめに、といった一般的な語にする |
| 話し言葉 | とりあえず、〜ましょう、〜ですね、大丈夫、！ | 事実を述べる敬体の文にする |
| 講義調のまとめ | 〜できるようになりました | 「以上で〜は完了です。次は〜へ進みます」にする |
| 対話的な主語・メタ発言 | あなた、私たち、このセクションでは〜を解説します | 機能や仕様を主語にし、前置きを省いて本題から書く |

<!-- textlint-enable no-ai-tone, ai-words-ja/no-ai-words -->

検査の対象は本文のテキストだけです。コードブロックとインラインコードは別のノードとして
扱われるため、コード例の中の語は検出されません。

### 語彙の検査（preset-ai-words-ja）

[preset-ai-words-ja](https://github.com/p1ass/textlint-rule-preset-ai-words-ja) は、AI が
書いた文章に多い単語を辞書で検出します。技術文書としては曖昧な語が対象です。指摘されたら、
何が起きるかを具体的に書き直してください。

<!-- 例として挙げる語は検出対象そのものなので、この段落だけ検査から外す。 -->
<!-- textlint-disable ai-words-ja/no-ai-words -->

対象になるのは「土台」「構図」「定番」「黙って」「事故」のような語です。たとえば
「間違った属性名は黙って無視されます」は「エラーを出さずに無視されます」に直します。

<!-- textlint-enable ai-words-ja/no-ai-words -->

辞書には、このドキュメントでは正確に使っている語も含まれます。次の5語は `.textlintrc.json`
の `allows` で除外してあります。

| 語 | 除外する理由 |
|---|---|
| 既定 | 「既定では」は本ドキュメントが採用している表記そのもの |
| 効く | 「キャッシュが効く」「属性が効かない」は技術文書として正確 |
| 漏れ | 「CSS のインポート漏れ」「`id` の指定漏れ」は原因を正確に指す |
| 部品 | 「UI の部品」として使っている |
| 検査 | 「リンクの検査」「文章の検査」として使っている |

除外を足すときは、その語がこのドキュメントで何を指しているかを上の表に書いてください。
言い換えられる語を `allows` に逃がすと、辞書を入れた意味がなくなります。

ルールを足す・ゆるめるときは、`tests/no-ai-tone.test.mjs` に「検出すべき文」と
「誤検出してはいけない文」を1件ずつ足してから `textlint-rules/no-ai-tone.js` を直し、
`npm test` で確かめてください。誤検出の例（助詞の「〜でも、」など）を残しておかないと、
正しい日本語まで弾くルールに育ちます。
