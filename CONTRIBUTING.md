...
テストがあるのは `textlint-rules/` のカスタムルールだけです。**ページが壊れていないことの
確認は `npm run build` が通ることで代用**します。後述のとおり、載せたコードが壊れていると
ビルドが落ちるようになっています。
...
`npm run lint:text` は、表記揺れ（`.textlint.prh.yml`）に加えて、公開ドキュメントに
ふさわしくない言い回しを `textlint-rules/no-ai-tone.js` で検出します。検出されるのは
次の4種類です。エラーが出たら、メッセージが示す書き方に直してください。

<!-- 表の「例」列は検出対象そのものなので、この表だけ検査から外す。 -->
<!-- textlint-disable no-ai-tone -->
