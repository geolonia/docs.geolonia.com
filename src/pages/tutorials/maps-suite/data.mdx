...
データの追加は、段1で覚えたエスケープハッチのパターン（**読み込みを待って `_getImpl()` で基盤の地図に触る**）を使います。ソース（データ）を登録し、レイヤ（描き方）を重ねます。
...
- **順番が大事**：`addLayer` が参照するソースは先に `addSource` してある必要があります。そして両方とも、地図の読み込み完了（`_whenReady()`）より後に呼びます。
...
- 「Style is not done loading」のようなエラー → `_whenReady()` を**待たずに** `addSource`/`addLayer` を呼んでいます。`await` を確認してください。
