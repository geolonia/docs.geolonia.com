...
description: 全地点を囲む範囲を計算し、データが変わっても画面に収まるようにします。
page: howto
badge: howto
---
...
- 地図の読み込み完了は `fitBounds()` の中で待つので、呼ぶ前に待つ必要はありません。
...
上のコードは、それぞれのタブの下でそのまま動いています。初期表示の `center` / `zoom` がどうであれ、読み込み後に全地点が収まる表示範囲に移動します。
