...
description: ズームや現在地、縮尺などのコントロールの表示と位置を指定します。
page: howto
badge: howto
---
...
<Lens name="suite">
maps-suite の初期化オプションで指定できるのは、既定のコントロールをまとめて消す `disableDefaultUI` だけです。個別に足したり位置を変えたりするときは、読み込みを待ってから基盤の地図（maplibre）に `addControl` します。

何も指定しないときは、**ナビゲーションと Geolonia のロゴが表示され、現在地・全画面・スケールは非表示**です。全部消したいなら次のように書きます。
...
`disableDefaultUI` は一括なので、「ナビゲーションだけ残す」のような指定はできません。その場合は一度すべて消してから、必要なものを `addControl` で足し直します。
...
- コントロールが出ない → 属性名や props 名のつづりを確認。間違った名前は、エラーを出さずに無視されます。
