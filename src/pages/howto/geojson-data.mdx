...
`addGeoJson()` が受け取るのはオブジェクトなので、URL から読むときは `fetch` を挟みます。点・線・面が混在していてもそのまま渡せます。色や線の太さは `map.data.setStyle({ fillColor: "#f00", strokeWeight: 2 })` のように指定します。
...
CSS のインポートなどのセットアップは [maps-suite チュートリアルの Step 0](/tutorials/maps-suite/first-map/) を参照してください。
...
- CSS のインポートなどのセットアップは [maps-react チュートリアルの Step 0](/tutorials/maps-react/first-map/) を参照してください。
