...
- **`data-style`**：Geolonia が用意するスタイル名（`geolonia/basic-v2`、`geolonia/midnight`、`geolonia/gsi` など）か、外部スタイル JSON の URL を指定できます。指定できる名前の一覧と、それぞれの見え方は[Geolonia Maps のスタイル一覧](/explanation/styles/)にあります。
...
```js
map.getGeoloniaMap().setStyle("https://tile.openstreetmap.jp/styles/osm-bright/style.json");
```

- **`map.getGeoloniaMap().setStyle(...)`**：スタイル JSON の URL か、インラインのスタイル定義オブジェクトを渡します。
...
- 属性を書いたのに反映されない → `data-style` のハイフンやつづりを確認。間違った属性名は、エラーを出さずに無視されます。
</Callout>
