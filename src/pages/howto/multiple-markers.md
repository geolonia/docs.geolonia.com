...
- [ ] 「複数地点を一度に出すには」を1行で示す（1点なら data-marker で十分、と対比）
- [ ] 地点データの持ち方を書く（配列 / GeoJSON、座標は [経度, 緯度] 順）
- [ ] embed で全マーカーを描く方法を書く（div の中に geolonia-marker を点の数だけ並べる）
- [ ] 実際に動く地図を載せて、全地点が出ることを確認する
- [ ] つまずき（座標の順序が逆／点が多すぎ→クラスタリング data-cluster／命令型は map.on('load') 内）
- [ ] 必要ならレンズ差（embed / suite / react）に触れる
-->
